...
function SignUp() {
    ...
    return (
        <div className='w-full h-[100vh] bg-[#1e1e2f] flex justify-center items-center' >
            ...
            <form className='w-[90%] h-[600px] max-w-[500px] bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col justify-center items-center gap-[20px] px-[20px]' onSubmit={handleSignUp}>

                <h1 className='text-white text-[30px] font-semibold mb-[30px]'>Register to <span className='text-blue-400'>Virtual Assitant</span></h1>

                <input type="text" placeholder='Enter your Name' className='w-full h-[60px] outline-none border-2 border-white bg-transparent text-white text-[18px] placeholder-gray-300 px-[20px] py-[10px] rounded-full' value={name} onChange={(e) => setName(e.target.value)} required />
                ...
                <button className='min-w-[150px] mt-[30px] h-[60px] bg-white rounded-full text-black font-semibold text-[19px]' disabled={loading}>{loading ? `Loading...` : `Sign Up`}</button>
                <p className='text-[white] text-[18px] cursor-pointer' onClick={() => navigate("/login")}>Already have an account ? <span className='text-blue-400'>Login</span></p>
